	...
	config: {
		...
		listeners: {
			beforeload: function(store, response, eOpts) {
				this.getProxy().setUrl('https://' + LoLChamps.app.REGION + LoLChamps.app.API_URL + LoLChamps.app.REGION + '/v1.4/summoner/by-name/' + LoLChamps.app.SUMMONER_NAME + '?api_key=' + KeyObject.getApiKey());
